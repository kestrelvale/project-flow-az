## [4.15.18] - 2026-09-25 (接力提示词自带交接身份、任务说明与下一步)

用户反馈（2026-09-25）：「你连原始的会话 ID 都没有放上去；也没有写清楚这个到底是什么；
下一步需要新会话去交接什么东西、执行什么呀？」

当时实测输出只有一句「本次交接身份：未指定」——整段里既没有**来源会话 thread id**，
也没有一句话说明**这是什么任务、当前什么状态**，更没有**新会话第一步做什么、验收标准是什么**。
更糟的是它**必然**如此：ticket 解析只看 `plan.md` 活跃区，而收工后卡已转入 `[-]` 待验收、
活跃区被清空——接力提示词的主战场恰恰是那一刻。

### Fixed（`flow-budget.py`）
- 新增 `handoff_fields()`：从 `flow/进展.md` 顶部交接棒取「来源会话 thread / 标题 / 现状 / 还剩 /
  卡在哪 / 下一步 / 台账」；`card_objective()` / `acceptance_of()` 从任务卡取目标与验收标准；
  `pending_specs()` 从台账取未回收规格点。
- 提示词新增三段：
  - **交接来源**：来源会话 `thread=<源 uuid>`、来源任务卡、来源交接棒标题、台账路径与未回收条数；
  - **任务说明**：一句话「这是什么（卡 objective）+ 当前状态（交接棒「现状」）+ 还剩 + 卡在哪」；
  - **下一步**：第一件事（交接棒「下一步」）+ 验收标准（卡 acceptance）+ 未回收规格点清单。
- 修 ticket 解析回归：覆盖 `[-]` 待验收区（活跃区为空时，唯一待验收卡可被认领；多于一张则要求显式指定）。
- 新增显式意图保护：意图里点名了某个编号时只用它——**哪怕该卡已归档，也只照实说「该卡不在活跃区」，
  绝不用别的卡兜底**（否则新会话会被带到另一张卡上开工）；同时该分支不输出字面路径（死路径契约）。

### Verification
- `tests/run-all.sh` 全量 **22 项 Exit 0**。
- 新增 `tests/test-relay-identity.py`（先红后绿）：断言提示词含源 thread id / 源任务卡 / 交接棒标题、
  「任务说明」段（含 objective 与现状）、「下一步」段（含验收命令与未回收计数）；
  断言卡在 `[-]` 待验收区时仍能解析（不再恒为「未指定」）；反向：两区都无卡时明确写「未指定」。
- 既有契约保持：`test-relay-multi-task.py`（已归档卡不给死路径、显式 `--ticket` 优先、并行队列与阻塞）、
  `test-relay-clarity.py`（工作根、不出现 `<ticket>` 占位符死路径）全绿。

### Packaging
- `VERSION` → `4.15.18`；项目侧 `flow/规范/VERSION` 由热同步接管。
