...
用户把接力提示词贴进**主仓**新会话，提示词里只有 `flow/plan.md` / `flow/tasks/<ticket>.md`
这类相对路径与占位符，**没有工作根、没有具体 ticket**；而主仓顶部的交接棒是另一条任务的
（DAILY-OPTIONS）。于是新会话先读错项目的 plan/进展，再自己去 `git worktree list`、
`flow/claims/`、源码里找「P4 到底在哪个工作区」—— 75 次工具调用遍历历史。
用户同时指出：「之前不是让把交接字数限制移除吗？已交接清楚任务为目的，不能省则省」。

### Fixed（接力提示词自包含，`flow-budget.py handoff_prompt()`）
- 提示词新增 **工作根绝对路径**，并声明「若当前 cwd 不是这个工作根，先 `cd` 过去；
  不要在别的 checkout 上开工」。
- 「只读取」清单从相对路径 + `<ticket>` 占位符改为**具体路径**：
  `<工作根>/flow/tasks/<真实ticket>.md`、`<工作根>/flow/specs/<真实ticket>.md`，
  并明确「只做这条卡对应的未回收规格点；不要遍历其它任务卡、其它工作区或历史归档」。
- 新增 `current_ticket()`：从顶部交接棒标题提取真实编号。**并修一个由此暴露的判据缺陷**——
  标题只带 `thread=<uuid>` 时，编号正则会从 UUID 尾部提取出假 ticket
  （实测得到 `a0c1cd-a264-7321-8779-8440`），生成指向不存在任务卡的提示词；
  现在先砍掉 `thread=` 之后的内容再匹配，识别不出才回退 `<ticket>` 占位符。

### Changed（交接棒不再有字节上限，`flow-boot.py` / `flow-gc.py` / 两份 SOP）
- 删除 `MAX_HANDOFF_BYTES`（4000）与对应阻塞项：**字节数不再是判据**，交接的第一优先级是
  让新会话看懂现场，逐页明细、证据清单该写就写。防「复述背景」仍由 `flow-distill` 的
  照抄检测承担（最长连续照抄 >= 400 字符或比例 >= 60%），结构底线仍由四字段 + `thread=` 承担。
- 同步 `references/会话预算与接力SOP.md`、`references/任务状态机与按需加载SOP.md`、
  `flow-gc.py` 注释；`tests/test-multi-project-structure.sh` 的旧断言
  「SOP 必须含 '1200 字节'」改为「必须写明『字节数不再是判据』」，防止文档与实现再次对不齐。

### Verification
- `tests/run-all.sh` 全量 **19 项 Exit 0**（新增 `test-relay-clarity.py`，先红后绿）。
- 该用例覆盖：①11KB 合规交接棒不再被体积拦截、但漏字段仍报「交接棒缺少字段」（撤判据 ≠ 撤校验）；
  ②`--print-relay` 输出的提示词含工作根绝对路径与具体 `flow/tasks|specs/<ticket>.md`，
  且只读取清单里不再出现 `<ticket>` 占位符；③标题无 ticket 时回退占位符、提示词仍可用。

### Packaging
- `VERSION` → `4.15.16`；项目侧 `flow/规范/VERSION` 由热同步接管，无需手工同步。

## [4.15.15] - 2026-09-24 (熔断回执自引用：接手的新会话不再被永久柔性阻塞)

事故现场：`zhengjie-hrm` 的 P4 分端会话（`01a0c1cd`，worktree `wt-p4`）在 09-24 07:07
落盘合规交接棒（带 `thread=01a0c1cd…`），07:08 开工时预算检查写出回执
`20260924-070803-stop.json` —— 回执的 `handoff_head` 快照**就是它刚写的那条标题**。
于是核销判据「顶部标题 != 快照」永远不成立；兜底扫描又被 `owner != declared` 挡在门外
（此时 `declared == owner`）。结果：这条回执对**任何接手的新会话**都是永久柔性阻塞 ——
熔断把「换新会话续跑」这个正确动作本身罚掉了，只剩继续用 67% 上下的旧上下文一条路。

### Fixed（`flow-boot.py` `read_pending_stop()`）
- 新增自引用判定：当 `handoff_head` == 顶部交接棒标题时，取**同目录上一条回执**的快照作对照，
  两者不同即证明「这轮确实落过新交接棒」→ 结合「顶部是当事会话的合规交接棒」判为已核销。
- 负向保持不变：熔断后**没写新交接棒**时，两个快照相同 → 仍判未交接、仍柔性阻塞（fail-closed 不放宽）。
- 不改软阻塞语义、不改交接棒格式、不自动改写/删除任何回执文件。
- 依据（磁盘证据，只读复验）：`wt-p4/flow/budget/` 5 条回执现在全部判为已核销
  （修复前 `20260924-070803-stop.json` 恒为未核销，且新会话开工即 `【柔性阻塞】`）。

### Verification
- `tests/run-all.sh` 全量 **18 项 Exit 0**（新增 `test-stop-receipt-selfref.py`，先红后绿：
  修复前该用例在新会话视角断言失败）。
- 用例覆盖两条时序：①先落交接棒、后写回执（快照撞自己）→ 新会话开工不阻塞；
  ②熔断后未落新交接棒（快照 == 顶部 == 上一条回执快照）→ 照旧柔性阻塞。
- 真机复验：`wt-p4` 逐条 `read_pending_stop(new_thread)` → 5/5 已核销，业务仓库只读未改动。

### Packaging
- `VERSION` → `4.15.15`；项目侧 `flow/规范/VERSION` 由热同步接管，无需手工同步。
