...
`flow-deliver.py` 会同时把汇报落盘为 `flow/deliveries/<时间>-<ticket>.json` 回执与 `flow/看板.md` 副本。**stdout 会随会话消失，落盘不会**——2026-09-22 的 o2o 实测：脚本跑了 7 次，用户可见回复里 0 次出现看板。回执让“没交付”变成下次开工必须点名的债务：`flow-boot.py` 会核对每个 `[-]` 待验收任务的回执，查无回执或编号无法识别都会在 `## 📨 交付回执` 与 `## ♻️ 回收建议` 里暴露。
