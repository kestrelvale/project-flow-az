...
require_text "README.md" "四阶段与驱动模式（唯一映射）"
require_text "README.md" "plan -> execute -> review -> handoff"
require_text "README.md" "引擎随仓库分发"
require_text "CHANGELOG.md" "### Packaging"
...
require_text "assets/templates/AGENTS.md" "四个分区标题与顺序是固定契约"
require_text "assets/templates/flow/plan.md" "## 🎯 当前聚焦待办 (P0)"
require_text "assets/templates/flow/plan.md" "## 📦 已完结归档 (Archived in flow/history/)"
require_text "assets/templates/flow/plan.md" "## 💡 本轮决策记录 (Decisions)"
require_text "scripts/flow-deliver.py" 'BOARD_SECTIONS'
...
require_text "assets/templates/flow/tasks/TEMPLATE.md" "schema: v2"
require_text "assets/templates/flow/tasks/TEMPLATE.md" "red_test:"
require_text "assets/templates/flow/tasks/TEMPLATE.md" "五拍循环"
require_text "assets/templates/flow/tasks/TEMPLATE.md" "按需深挖"
require_text "references/驱动模式与验证策略.md" "NO PRODUCTION CODE WITHOUT A FAILING TEST FIRST"
require_text "references/驱动模式与验证策略.md" "to-spec"
require_text "references/驱动模式与验证策略.md" "按需"
require_text "assets/templates/AGENTS.md" "交付验收卡"
...
# 交接三段链必须在 SOP 里写全，且与实现字段一致；防止文档与代码再次对不齐。
require_text "references/会话预算与接力SOP.md" "三段链"
require_text "references/会话预算与接力SOP.md" "现状:"
require_text "references/会话预算与接力SOP.md" "还剩:"
require_text "references/会话预算与接力SOP.md" "卡在哪:"
require_text "references/会话预算与接力SOP.md" "下一步:"
# 交接以「说清楚」为目的：字节上限已被撤掉（v4.15.16），改为要求文档显式写明「不设字节上限」。
require_text "references/会话预算与接力SOP.md" "字节数不再是判据"
require_text "references/会话预算与接力SOP.md" "降级行为"
