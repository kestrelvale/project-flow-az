...
由 `初始化SOP.md` 执行：复制脚本 → `chmod +x .hooks/stop-doccheck.sh` → 合并两端配置 → 检查 Codex 版本与 hooks 开关 → 提醒 trust 项目及 `/hooks` 批准。
---

## 5. 执行完整性硬门禁与防半路收工 (Execution Completeness Gate)

### 5.1 为什么会发生“半路收工”？
当 Agent 在长链路复合任务中只编写了调研/分析文档便过早停止调用工具尝试回复时，Codex 的底层 Stop Hook 会如期触发并注入 `【收工自检】`。如果 Agent 此时机械地把自检当成“完结信号”而追加进展日志，就会导致**代码一行没改却宣告收工**的严重流程漂移。

### 5.2 自检反查铁律 (Stop Hook Verification Checklist)
收到 `【收工自检】` 提示词时，Agent **必须首先执行以下三项反查**：
1. **代码落地反查**：用户 Prompt 要求的代码修改、Bug 修复是否已落盘？（纯写方案文档 = 0分未完成）；
2. **Sub-Agent 状态反查**：派发的子 Agent 是否已全部 `completed` 并完成本地合并测试？
3. **未完成熔断机制**：若存在任何未落盘的代码任务，**严禁向 `flow/进展.md` 追加完结日志，必须立即继续调用工具完成修改与自测**！
