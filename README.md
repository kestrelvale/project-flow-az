...
任何实质性交互第一屏均输出极简智能标签看板，自适应所有设备与终端：

```markdown
### 📊 任务状态看板
- 🎯 **当前聚焦 (P0)**: `[ ]` 微信支付退款与结算单全链路闭环
- ⏳ **待人工验收**: `[-]` 财务对账报表 Web 端重构 `[👤 待人工验收]`
- 🤖 **自动验收通过**: `[-]` 微信支付分账回调异步断言测试 `[🤖 自动测试通过]`
- 📦 **已归档完结**: `[✓]` 基础架构迁移、SKU品类池搭建已移入 flow/history/
- 💡 **本轮决策**: `[⚡ 自动决策]` 支付超时时间设为 15 分钟，超时自动释放冻结库存
