...
> 基于开源协作框架深度魔改的 **AI 多 Agent 协作与任务闭环管理体系（四状态机与即时回收/决策分级版）**。
> 彻底解决“历史任务死循环复读、缺乏即时回收机制、上下文全量污染、旧计划绑架当面指令、流程管理漂移”等工程治理弊端。兼容 `project-flow-az` 与 `project-flow-cy` 双别名触发。
...
- **双轨决策与升级门机制 (Decision Gate)**：局部实现细节 Agent 自动决策并记录；架构变更/破坏性改动/需求冲突触发人工决策卡。
...
| `references/任务状态机与按需加载SOP.md` | 任务四状态机、闭环调度、按需加载与决策机制 |
...
- **双轨决策与升级门**：内部代码细节 Agent 自动决策并记入 `decisions.md`；架构破坏/高风险改动触发人工决策卡。
